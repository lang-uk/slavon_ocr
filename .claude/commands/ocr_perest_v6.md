...
- ъ (hard sign): mixed word-final usage — see the TRAILING-ъ SWEEP in STEP 3. Decide by glyph only.
...
- For word-final ъ, decide by glyph only — see the TRAILING-ъ SWEEP in STEP 3. Both directions (adding a ъ that isn't there and dropping one that is) are equally wrong.
