...
Then do a dedicated є/е SWEEP (this is the single most frequent error class):
1. Find every е or є in your transcription.
2. Look at the corresponding handwritten glyph on the card.
3. Decide by SHAPE, not by word:
   - є (U+0454) is written like a backwards C with a horizontal middle bar — the opening faces RIGHT. Looks like an upside-down 3.
   - е (U+0435) is written as a modern Ukrainian "e" — a closed loop with the opening on the LEFT.
4. NEVER decide based on which looks "more correct" in modern Ukrainian. The same card can have both letters in neighboring words (нѣкоторыє but епископове on a single card). Modern spelling priors are a TRAP — they are wrong more often than they are right on these cards.
5. If still uncertain after looking at the glyph, write what your eyes see and flag the word in "notes". Do NOT silently default to either letter.

If you find errors (from any of the above checks), fix them before saving. When genuinely uncertain about a character, put your best reading and note the uncertainty in "notes".
...
VOWEL LETTERS
- є (U+0454) vs е (U+0435): both valid, often coexist on a single card. Decide by GLYPH SHAPE — see the dedicated є/е sweep in STEP 3. Never default either way and never decide by modern-Ukrainian word priors.
...
- NEVER SUBSTITUTE a familiar word for an unfamiliar one (сыномъ not сүнодъ, исперва not неперва).
- є vs е and ү vs у are DIFFERENT letters — the є/е sweep in STEP 3 is mandatory.
